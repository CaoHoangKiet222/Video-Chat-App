      ...
      <Container>
         <MainLayout>
            ...
            <Routes>
               ...
               <Route path="/Friends/*" element={<SideBars header="Friends" display="none" />} />
            ...
            <Routes>
               {
                  conversation?.conv.map(({members, messages, _id: id}) => {
                     ...
                     return <Route path={`/Chats/${encodeURIComponent(member.name)}`} key={id} element={
                        <InfoBar
                           room={id}
               ...
               {
                  friends?.map(friend => {
                     return <Route path={`/Friends/list-friends/${encodeURIComponent(friend.name)}`} key={friend._id} element={
