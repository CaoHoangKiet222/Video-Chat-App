...
import {Avatar, ChatContent, ChatInfo, ChatItem, ChatText} from "./ChatItems.styled";
...
const ChatItems = (props) => {
   ...
   if (props.header === "Friends") {
      pathname = `/video-chat/${props.header}/list-friends/${encodeURIComponent(member.name)}`;
   }
   else pathname = `/video-chat/${props.header}/${encodeURIComponent(member.name)}`;

   useEffect(() => {
      if (member && user) {
         socket.emit("joinVideo", {user, friend: props.member});
         // return () => {
         //    socket.disconnect();
         //    socket.off();
         // }
   ...
   return (
      <>
         {
            props.header === "Friends" && props.isDiff && <li>{member.name[0].toUpperCase()}</li>
         }
         <ChatItem>
            <Link to={pathname}>
               <Avatar>
               ...
               <ChatContent>
                  <ChatInfo>
                     <h6>{member.name}</h6>
                     {
                        props.header === "Chats" && <div>{formatDate(props.messageDate)}</div>
                     }
                  </ChatInfo>
